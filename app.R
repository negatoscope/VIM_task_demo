...
ui <- fluidPage(
  ...
  conditionalPanel(
    ...
    fluidRow(
      column(4,
             ...
             actionButton("selectButton", "Select", width = "100%")
      ),
      column(8,
  ...
  conditionalPanel(
    ...
    radioButtons("dreamLocation", "Was your dream scene located indoors or outdoors?",
                 choices = c(
                   "Indoors" = 0,
                   "Outdoors" = 1
                 )),
...
server <- function(input, output, session) {
  ...
  randomImage <- reactiveVal(sample(1:15, 1))
  ...
  observeEvent(input$startButton, {
    ...
    currentPhase(2)
    randomImage(sample(1:15, 1))
    updateSliderInput(session, "progressBar", value = randomImage())
  })
  ...
  observeEvent(input$submitButton, {
    ...
    results <- data.frame(
      ...
      DreamLocation = input$dreamLocation,
    ...
    updateRadioButtons(session, "dreamLocation", selected = character(0))
